...
sys_open(void)
{
  ...
  /* find target file if this is a sympolink and not set O_NOFOLLOW */
  /* i.e. find correct ip*/
  int depth = 0;
  while (ip->type == T_SYMLINK && !(omode & O_NOFOLLOW)) {
    ...
    iunlockput(ip);
    if ((ip = namei(ktarget)) == 0) {     // get inode according to name
...
sys_symlink(void)
{
  ...
  // write ktarget's content to inode's blocks specified by ip->addrs
